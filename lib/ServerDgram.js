...
class ServerDgram {
  constructor(options) {
    ...
    const { rpcServer, bindAddress, bindPort } = options;
    privateData.set(this, { rpcServer, onListen, onMessage });
    if (bindPort) {
      this.createSocket({ bindAddress, bindPort });
    }
  }

  createSocket({ bindAddress, bindPort }) {
    this.closeSocket();
    ...
    const bindArgs = [bindPort];
    if (bindAddress) {
      bindArgs.push(bindAddress);
    }
